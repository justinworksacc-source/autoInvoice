...
async function readStore() {
  ...
  const [clients] = await db.execute(
    `SELECT LOWER(billing_email) id, customer_name name, billing_email email,
     ...
     FROM monthly_invoice_clients WHERE company_id = 1 ORDER BY id`
  );
...
export default async function handler(req, res) {
  try {
    ...
    else if (input.action === "delete_client") {
      const email = String(input.client_id || "").toLowerCase();
      await database().execute("DELETE FROM monthly_invoice_payments WHERE company_id=1 AND billing_email=?", [email]);
      await database().execute("DELETE FROM monthly_invoice_clients WHERE company_id=1 AND billing_email=?", [email]);
